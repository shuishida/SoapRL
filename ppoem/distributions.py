...
class DiagGaussianModule(nn.Module):
    def __init__(self, output_space: spaces.Box, latent_dim: int, log_std_init: float = 0.0, option_dim: Optional[int] = None, **kwargs):
        ...
        self.dist = DiagGaussianDistribution(action_dim if option_dim is None else action_dim * option_dim)
        self.net, self.log_std = self.dist.proba_distribution_net(
            latent_dim=latent_dim, log_std_init=log_std_init
        )

    def forward(self, latent_pi) -> DiagGaussianDistribution:
...
class SquashedDiagGaussianModule(nn.Module):
    ...
    def forward(self, latent_pi) -> SquashedDiagGaussianDistribution:
        mean_actions = self.net(latent_pi)
        action_std = th.ones_like(mean_actions) * self.log_std.exp()
        dist = deepcopy(self.dist)
        if self.option_dim is not None:
...
class CategoricalModule(nn.Module):
    ...
    def forward(self, latent_pi) -> CategoricalDistribution:
...
class MultiCategoricalModule(nn.Module):
    ...
    def forward(self, latent_pi) -> MultiCategoricalDistribution:
        action_logits = self.net(latent_pi)
        dist = deepcopy(self.dist)
        categoricals = []
...
class BernoulliModule(nn.Module):
    ...
    def forward(self, latent_pi) -> BernoulliDistribution:
        action_logits = self.net(latent_pi)
        dist = deepcopy(self.dist)
        if self.option_dim is not None:
            action_logits = rearrange(action_logits, "b (o a) -> (b o) a", o=self.option_dim)
...
class StateDependentNoiseModule(nn.Module):
    ...
    def forward(self, latent_pi) -> ModularStateDependentNoiseDistribution:
        mean_actions = self.net(latent_pi)
...
class DistributionModule(nn.Module):
    ...
    def forward(self, latent_pi) -> Distribution:
        return self.dist_module(latent_pi)
...
class EmbedActions(nn.Module):
    def __init__(self, output_space: spaces.Space):
        super().__init__()
        self.output_space = output_space
        if isinstance(output_space, spaces.Box):
            self.output_dim = int(np.prod(output_space.shape))
        elif isinstance(output_space, (spaces.Discrete, spaces.MultiBinary)):
            self.output_dim = int(output_space.n)
        elif isinstance(output_space, spaces.MultiDiscrete):
            self.output_dim = sum(output_space.nvec)
        else:
            raise NotImplementedError(f"{output_space} space is not supported")

    def forward(self, action):
        if isinstance(self.output_space, spaces.Discrete):
            return F.one_hot(action, self.output_dim).float()
        elif isinstance(self.output_space, spaces.MultiDiscrete):
            return th.cat([F.one_hot(action[..., i], size).float() for i, size in enumerate(self.output_space.nvec)], dim=-1)
        return action


def create_mixed_distribution(distribution: Distribution, option_prob: th.Tensor) -> Distribution:
    """
    :param distribution: torch.distributions.Distribution (B O) ...
    :param option_prob: B O (probability sums to 1 for each batch)
    :return: torch.distributions.Distribution B A
    """
    assert option_prob.sum(dim=-1).allclose(th.ones_like(option_prob[..., 0]))
    batch_size = option_prob.shape[0]
    return_dist = deepcopy(distribution)
    if isinstance(distribution, CategoricalDistribution):
        probs = rearrange(distribution.distribution.probs, "(b o) a -> b o a", b=batch_size)
        probs = th.einsum("boa,bo->ba", probs, option_prob)
        return_dist.distribution = Categorical(probs=probs)
        return return_dist
    elif isinstance(distribution, (DiagGaussianDistribution, SquashedDiagGaussianDistribution,
                                   StateDependentNoiseDistribution, ModularStateDependentNoiseDistribution)):
        raise NotImplementedError
    elif isinstance(distribution, BernoulliDistribution):
        probs = rearrange(distribution.distribution.probs, "(b o) a -> b o a", b=batch_size)
        probs = th.einsum("boa,bo->ba", probs, option_prob)
        return_dist.distribution = Bernoulli(probs=probs)
        return return_dist
    elif isinstance(distribution, MultiCategoricalDistribution):
        categoricals = []
        for categorical in distribution.distribution:
            probs = rearrange(categorical.probs, "(b o) a -> b o a", b=batch_size)
            probs = th.einsum("boa,bo->ba", probs, option_prob)
            categoricals.append(Categorical(probs=probs))
        return_dist.distribution = categoricals
        return return_dist
