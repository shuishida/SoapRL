...
class PPORolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.advantage_matrix = th.zeros((self.buffer_size, self.n_envs, self.option_channels, self.option_dim, self.option_dim), dtype=th.float)        # p(a, z'| s, z)
        ...
        self.option_cond_probs = th.zeros((self.buffer_size, self.n_envs, self.option_channels, self.option_dim, self.option_dim), dtype=th.float)   # pi(z_t, z_{t+1} | \tau)
    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor, save_dir) -> None:
        ...
        self.option_cond_probs = self.option_joint_probs / self.option_joint_probs.sum(dim=-1, keepdim=True)  # p(z' | z, \tau) = p(z, z') / \sum_{z'} p(z, z')
        ...
        for step in reversed(range(self.buffer_size)):
            ...
            self.advantages[step] = last_gae_lam = th.einsum("bcij,bcij->bci", self.option_cond_probs[step], advantage_matrix)  # (B, Z)
