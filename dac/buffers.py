...
class PPORolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.episode_starts = th.zeros((self.buffer_size, self.n_envs), dtype=th.float)
        ...
        self.returns = th.zeros((self.buffer_size, self.n_envs), dtype=th.float)
        self.advantages = th.zeros((self.buffer_size, self.n_envs), dtype=th.float)

        self.option_returns = th.zeros((self.buffer_size, self.n_envs), dtype=th.float)
        self.option_advantages = th.zeros((self.buffer_size, self.n_envs), dtype=th.float)

        self.log_probs = th.zeros((self.buffer_size, self.n_envs), dtype=th.float)

        self.prev_options = th.zeros((self.buffer_size, self.n_envs, self.option_dim), dtype=th.float)

        self.options = th.zeros((self.buffer_size, self.n_envs, self.option_dim), dtype=th.float)
        self.option_probs = th.zeros((self.buffer_size, self.n_envs, self.option_dim), dtype=th.float)

        self.next_options = th.zeros((self.buffer_size, self.n_envs, self.option_dim), dtype=th.float)
        self.next_options_probs = th.zeros((self.buffer_size, self.n_envs, self.option_dim), dtype=th.float)

        super().reset()

    def compute_returns_and_advantage(self, last_values: th.Tensor, last_options: th.Tensor, last_options_probs: th.Tensor, save_dir) -> None:
        """
        ...
        self.next_options[-1] = last_options.cpu().clone().detach()
        self.next_options_probs[-1] = last_options_probs.cpu().clone().detach()

        last_gae_lam = 0
        for step in reversed(range(self.buffer_size)):
            next_values = th.einsum("eo,eo->e", self.next_values[step], self.next_options[step]) * (1.0 - self.terminals[step])   # (N,)
            target_values = self.rewards[step] + self.gamma * next_values + self.gamma * self.gae_lambda * (1.0 - self.dones[step]) * last_gae_lam   # V_target (B,)
            self.advantages[step] = last_gae_lam = target_values - th.einsum("eo,eo->e", self.values[step], self.options[step])  # (B, Z)
    ...
    def add(
        ...
        next_option: Optional[th.Tensor] = None,
        next_option_prob: Optional[th.Tensor] = None,
    ) -> None:
        """
        ...
        self.prev_options[self.pos] = prev_option.cpu().clone().detach()
        self.options[self.pos] = option = option.cpu().clone().detach()
        self.option_probs[self.pos] = option_prob = option_prob.cpu().clone().detach()

        if next_value is not None:
            self.next_values[self.pos] = next_value.cpu().clone().detach()
        if next_option is not None:
            self.next_options[self.pos] = next_option.cpu().clone().detach()
        if next_option_prob is not None:
            self.next_options_probs[self.pos] = next_option_prob.cpu().clone().detach()

        if self.pos > 0:
            dones = repeat(self.dones[self.pos-1].bool(), "b -> b o", o=self.option_dim)
            self.next_values[self.pos-1] = th.where(dones, self.next_values[self.pos-1], value)
            self.next_options[self.pos-1] = th.where(dones, self.next_options[self.pos-1], option)
            self.next_options_probs[self.pos-1] = th.where(dones, self.next_options_probs[self.pos-1], option_prob)
    ...
    def _get_samples(self, batch_inds: th.Tensor, env_inds: th.Tensor, env: Optional[VecNormalize] = None) -> PPORolloutBufferSamples:
        ...
        return PPORolloutBufferSamples(
            ...
            episode_starts=select(self.episode_starts),
