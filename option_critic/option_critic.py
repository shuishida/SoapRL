...
class OptionCriticFeatures(nn.Module):
    ...
    def get_state(self, obs, unsqueeze=False):
        if unsqueeze:
            obs = obs.unsqueeze(0)
        if obs.ndim == 1:
            obs = F.one_hot(obs.long(), self.in_features).float()
        obs = obs.to(self.device)
        state = self.features(obs)
        return state
...
def actor_loss(obs, option, logp, entropy, reward, done, next_obs, model, model_prime, args):
    state = model.get_state(to_tensor(obs), unsqueeze=True)
    next_state = model.get_state(to_tensor(next_obs), unsqueeze=True)
    next_state_prime = model_prime.get_state(to_tensor(next_obs), unsqueeze=True)
