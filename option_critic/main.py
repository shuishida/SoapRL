...
register(
    id="segskill-TMaze-v0",
    entry_point='customenvs.tmaze:CorridorEnv',
...
def run(args):
    ...
    option_critic = option_critic(
        in_features=env.observation_space.n if isinstance(env.observation_space, gym.spaces.Discrete) else env.observation_space.shape[0],
    ...
    while steps < args.max_steps_total:
        ...
        obs   = env.reset()
        state = option_critic.get_state(to_tensor(obs), unsqueeze=True)
        ...
        while not done and ep_steps < args.max_steps_ep:
            ...
            state = option_critic.get_state(to_tensor(next_obs), unsqueeze=True)
