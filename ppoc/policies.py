...
class PPOActorCriticPolicy(ActorCriticPolicy):
    ...
    def forward(
        self,
        obs: PyTorchObs,
        last_option: Optional[th.Tensor],
        ...
        option = self.init_options(len(episode_starts), latent_obs)
        if last_option is not None:
            option = th.where(repeat(episode_starts, "b -> b o", o=self.option_dim).bool(), option, last_option)
        assert th.allclose(option.sum(dim=-1), th.ones_like(option.sum(dim=-1))), "Option probabilities must sum to 1"

        values = self.value_net(latent_vf)
        distribution = self.action_dist(latent_pi, option)
        actions = distribution.get_actions(deterministic)

        log_prob, log_option_prob, switch_prob = self.compute_transitions(distribution, actions, latent_obs)
        switch_prob = th.where(episode_starts.bool(), th.ones_like(switch_prob), switch_prob)
        with th.no_grad():
            switch = th.bernoulli(switch_prob).unsqueeze(-1)
            new_option = (1 - switch) * option + switch * F.gumbel_softmax(log_option_prob, hard=True)
        return actions, values, log_prob, log_option_prob, switch_prob, option, new_option
    ...
    def compute_transitions(self, distribution: Distribution, actions: th.Tensor, latent_obs: th.Tensor):
        log_prob = distribution.log_prob(actions)
        log_option_prob = self.option_policy(latent_obs)  # log pi(z|s): shape (B, O)
        switch_prob = self.switch_prob(latent_obs).squeeze(-1)       # p(switch|s): shape (B, 1)
        return log_prob, log_option_prob, switch_prob
    ...
    def predict_values(self, obs: PyTorchObs, option) -> th.Tensor:
        """
    ...
    def _predict(
        ...
        """
        actions, values, log_prob, log_option_prob, switch_prob, options, new_option = self(observation, last_option=last_option, episode_starts=episode_starts, deterministic=deterministic)
    ...
    def predict(
        ...
        with th.no_grad():
            if state is None:
                ...
                option = th.tensor(state, dtype=th.float32, device=self.device)
            ...
            actions, option = self._predict(
                observation, last_option=option, episode_starts=episode_start, deterministic=deterministic
            )
