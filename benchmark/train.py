...
import sys
sys.path.append(".")
from common.utils.train_utils import VideoRecorderCallback
from benchmark.exp_manager import MyExperimentManager as ExperimentManager
from benchmark.utils import ALGOS


for episode_length in range(21):
    env_id = f"corridor-l{episode_length}"
    register(
        id=env_id,
        entry_point='common.envs.corridor:CorridorEnv',
        max_episode_steps=episode_length + 1,
        kwargs={
            "width": episode_length,
        }
    )
...
def train() -> None:
    ...
    conf_file = str(Path(__file__).parent / f"config/{args.algo}.yml")
    if args.conf_file is not None or not os.path.exists(conf_file):
        conf_file = args.conf_file

    print("Using config at:", conf_file)
    ...
    if args.eval_env_kwargs is not None:
        eval_env_kwargs.update(args.eval_env_kwargs)
    ...
    if results is not None:
        ...
        if model is not None:
            if args.render_freq > 0:
                eval_env = exp_manager.create_envs(n_envs=1, eval_env=True)
                exp_manager.callbacks.append(VideoRecorderCallback(eval_env, render_freq=args.render_freq))
